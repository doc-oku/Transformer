...
class LayerNorm:
    ...
    def backward(self, x):
        self.delta = x.reshape(-1, self.dimension)
        d_xb = self.gamma * self.delta
        d_var = np.sum(self.d_input * d_xb, axis=1)
        d_var = d_var.reshape(-1, 1)
        d_var *= -0.5 / self.buf3

        x1 = np.sum(d_xb, axis=1)
        x1 = x1.reshape(-1, 1)
        x1 /= -self.buf1

        x2 = np.sum(self.d_input, axis=1)
        x2 = x2.reshape(-1, 1)
        x2 *= -2.0 * d_var / self.dimension
        d_ave = x1 + x2

        x = d_xb / self.buf1
        x += (2.0 * self.d_input * d_var + d_ave) / self.dimension
        x = x.reshape(self.inputs_shape)
        return x
